...
public class MARS
{
    ...
    public BigInteger Encrypt(BigInteger ZOV)
    {
        ...
        return ArrayToNum(new uint[] { A, B, C, D });
    }
...
class Program 
{
    ...
    {
        MARS MARS = new MARS();
        BigInteger Key  = GenNum(128); //128-448 bit (step 32 bits);
        BigInteger Text = GenNum(128); //128 bit;

        MARS.KeySetup(Key);
        BigInteger NewText = MARS.Encrypt(Text);
        BigInteger OldText = MARS.Decrypt(NewText);
